...
class _Tree:
    ...
    def __init__(self, raw_submission):
        ...
        for comment in comments:
            ...
            self.wrapped_comments[wrapped_comment.parent_id].append(
                wrapped_comment)
    ...
    def _recursive_node(self, parent_id):
        ...
        for child in children:
            self.tree.create_node(child.id, child.id,
                                  parent=parent_id, data=child)

            if child.depth < 0:
                logger.error(
                    f"Node {child.id} has invalid depth of {child.depth}!")
                raise ValueError(
                    f"Node {child.id} has invalid depth of {child.depth}!")
