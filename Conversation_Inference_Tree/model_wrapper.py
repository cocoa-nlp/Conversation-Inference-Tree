...
from .logger import log_progress, logger
...
class _ModelWrapper:
    ...
    def __init__(self, model_name: str, model_origin: str, model_params: dict = None):
        ...
        if model_origin == "hf":
            ...
            try:
                ...
                raise KeyError(
                    "Login with token failed!  Make sure to set your huggingface login key under 'token' in your .env!")
            ...
            self.model = pipeline("text-generation", model=automodel,
                                  tokenizer=autotokenizer, pad_token_id=autotokenizer.eos_token_id)
            ...
            logger.error(
                "model origin selected incorrectly, failed to load model")
    ...
    def _get_gpu_with_most_free_memory(self):
        ...
        try:
            ...
            best_gpu = sorted(
                free_memories, key=lambda x: x[1], reverse=True)[0][0]
            return str(best_gpu)
        except:
            logger.info(
                "No CUDA device found, skipping device finding function.")
            return None
    ...
    def generate(self, input: str, agent: _Agent):
        ...
        if input == '':
            raise ValueError(
                "Text input was found to be empty when constructing prompt!")
        formatted_input = agent.form_prompt(input)

        if self.model_origin == "hf":
            response = self.model(formatted_input, return_full_text=False)[
                0]["generated_text"]
            log_progress.info(
                f"Prompt: {formatted_input} GAVE OUTPUT {response}")

            if re.match(r".*\btext\b(?:\s+\b\w+\b){0,5}\s+\bsummarize\b.*", response):
                logger.error(
                    f"An empty input was detected by the LLM in the following entry: '{input}.'  The response the model gave was: '{response}.'")
            ...
            logger.info(
                f"Prompt: {formatted_input} GAVE OUTPUT {response['choices'][0]['message']['content']}")
